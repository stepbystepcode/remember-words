<template>
  <div id="q-app">
    <div class="q-layout q-layout--standard" style="min-height: 274px">
        ...
        <div class="header row items-center justify-between q-px-md">
          <button
            ...
            class="
              ...
              non-selectable
              no-outline
              q-btn--flat q-btn--round q-btn--actionable
              ...
              ><span
                ...
                ><i
                  ...
                  >
                    ...
                    <line x1="5" y1="12" x2="11" y2="6"></line></svg></i></span
            ></span>
          </button>
            ...
            >
              ...
              ><span class="q-btn__wrapper col row q-anchor--skip"
                ><span
                  ...
                  "
                  ><div>
                    {{ moments("M") }}月{{ moments("D") }}日 {{ moments("T") }}
                  </div></span
                ></span
              >
            </button>
              ...
              ><span
                ...
                ><i
                  ...
                  >
                    ...
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                    <path d="M5 12l5 5l10 -10"></path></svg></i></span
            ></span>
          </button>
          <div
            role="progressbar"
            aria-valuemin="0"
            aria-valuemax="100"
            class="q-circular-progress q-circular-progress--indeterminate"
          ...
          >
            ...
            >
              <circle
                ...
                r="50"
                class="q-circular-progress__circle text-primary"
              ></circle>
            </svg>
          </div>
        </div>
      ...
      <form
        action="https://www.codingboy.xyz/insert2.php"
        method="post"
      ...
      >
        <div class="content list-content">
          <div class="select">
            ...
            <button @click="confirmPerson()" type="button" class="set">
              确定
            </button>
          </div>
          <label for="file" class="custom-file-upload"> +添加图片 </label>
          <input
            type="file"
            id="file"
            name="file"
            style="display: none"
            onchange="if(document.querySelector('select').value!=0){this.form.submit()}else{swal('请选择身份！')}"
          />
        </div>
        <input
          type="text"
      ...
      <div>
        <ul class="top-ul">
          <li v-for="(person, index) in today" :key="index" class="top-li">
            <i
              ><img
                :src="`https://www.codingboy.xyz/${index + 1}.png`"
                alt="" /></i
            ><span class="person">{{ this.lists[person].person }}</span>
            <div style="flex-grow: 1"></div>
            {{ this.topTime[index]
            }}<img
              :src="`https://www.codingboy.xyz/avatar/${this.lists[person].person}.jpg`"
              class="avatar"
              alt=""
            />
          </li>
...
export default {
  ...
  mounted() {
    this.axios.get("https://www.codingboy.xyz/fetch2.php").then((res) => {
      this.lists = res.data;
      console.log(this.lists);
      for (let i = 0; i < this.lists.length; i++) {
        let date = new Date(this.lists[i].time * 1000);
        let date2 = new Date();
        if (date2.getDate() == date.getDate() && this.today.length < 3) {
          this.today.push(i);
          this.topTime.push(
    ...
    if (document.cookie != "") {
      ...
      for (let i = 0; i < ca.length; i++) {
        let c = ca[i].trim();
        document.querySelector("select").value = c.substring(
          name.length,
          c.length
        );
      }
  ...
  methods: {
    confirmPerson() {
      if(document.querySelector("select").value==0){swal("请选择身份！")}else{
      swal(
        "是否选择" +
        ...
        if (value == "yes") {
          document.cookie = "user=" + document.querySelector("select").value;
          document.querySelector(".select").style.display = "none";
        }
      });}
    },
    ...
    submit: () => {
      ...
      axios.post("https://www.codingboy.xyz/insert2.php", this.form, {
        headers: {
...
.q-btn.q-btn-item.non-selectable.no-outline.q-btn--standard.q-btn--round.bg-primary.text-white.q-btn--actionable.q-focusable.q-hoverable.q-btn--wrap
  ...
  background-color: #52a2c4 !important;
}
.material-icons {
...
:root {
  --q-color-primary: #52a2c4;
  --q-color-secondary: #26a69a;
  --q-color-accent: #52a2c4;
...
.text-primary {
  color: #52a2c4 !important;
  color: var(--q-color-primary) !important;
}

.bg-primary {
  background: #52a2c4 !important;
...
.bg-normal {
  box-shadow: 0px 1px 8px 1px rgba(0, 0, 0, 0.06666666666666667);
}
...
.login-card {
  ...
  border-radius: 8px;
  padding: 58px;
  box-shadow: 0px 2px 4px 2px rgba(0, 0, 0, 0.06666666666666667);
}
...
body.body--light .color-bg-content,
body.body--light .color-bg-normal {
  background-color: #fff;
}

body.body--light .color-bg-list {
  background-color: #fafafa;
}

body.body--light .color-text {
  color: #333;
}

body.body--light .color-desc {
  color: #a2a2a2;
}
...
body.body--dark .color-bg-normal {
...
body.body--dark .color-desc {
  color: #b2b2b2;
}

body.body--dark .color-divider {
  color: #292828;
}
